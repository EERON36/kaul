...
The client is the central entity in Kaul. Journal entries, assignments, goals,
follow-ups, documents, and monthly reports are organised around the client.

A Version 1 client is created with:

- First name
- Last name
- Person identifier
- Category
- Optional Swedish personal identity number
- Optional placing unit and legal basis
- Optional responsible social worker name, phone number, and email address

The social-worker phone number and email address describe the responsible
social worker, not the Client. The Swedish personal identity number is a
separate sensitive value and never replaces the organisation-local person
identifier.
...
- Many follow-ups
- Many documents
- One monthly-report lineage per calendar month, with retained signed revisions
...
- The Version 1 person identifier is an opaque organisation-local reference,
  ...
  organisation.
- A Swedish personal identity number is optional, has no uniqueness rule in
  this slice, is not an application identifier, and is excluded from ordinary
  lists, search, URLs, logs, and audit metadata. It is read only through a
  narrow current-Client-authorisation boundary and edited only through the
  existing Client-modification permission.
...
- Entry type
- Event date and time
- A content format and either historically preserved legacy narrative content
  or six structured content sections
...
- New Anteckningar use six simultaneously visible plain-text sections: Hälsa,
  Utbildning/Sysselsättning, Känslor och Beteende, Sociala relationer,
  ADL/självständighet, and Övrigt. At least one section contains meaningful
  text; no individual section is mandatory.
- Existing signed legacy narrative content is never rewritten. Existing legacy
  drafts may be presented under Övrigt and convert only when their authorised
  author saves the structured form.
...
A monthly report is a manually authored Client deliverable for one explicit
calendar year and month. It is not generated from Journal records in this
slice.
...
- Six structured sections matching the Anteckning vocabulary
- Shared draft version
- Stable report reference and lineage revision
- Creator and latest draft updater
- Status: `DRAFT` or `SIGNED`
- Signing timestamp and historical signer snapshot
- Optional direct link to the signed report it replaces

### Monthly Report Rules

- There is one canonical lineage per Organisation, Client, and calendar month.
- A draft is shared by currently authorised users, not author-private.
- Saving persists the draft and increments its optimistic version; it is not a
  separate lifecycle state.
- Signing requires meaningful content in at least one section, preserves the
  reviewed payload, and commits immutable audit evidence atomically.
- A signed report is immutable at both application and PostgreSQL levels.
- A correction is a new directly linked draft and signed revision. The
  preceding signed report remains permanently readable and cannot branch into
  multiple replacements.
- Historical creation does not grant Client access. Current Organisation,
  Client lifecycle, and Assignment rules apply to every read and mutation.
- Automatic Journal synthesis, approval chains, and unrelated duplicate
...
Monthly Report
├── One Client and Calendar Month
├── Shared Draft and Current Authorised Editors
├── Optional Signer
└── One Root and Optional Sequential Replacement Revisions
```
